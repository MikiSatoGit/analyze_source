...
##### set title
	...
	if debug_out:
		for index1 in range(0,len(proc_codes)):
			print '<analyze_function_codes (pre)',
...
#	goto 					jump
	...
	previous_end_index1 = -1
	previous_end_index2 = -1
...
# reverse loop until previous main process(not ???)	
						if  index1_r==previous_end_index1 and index2_r==previous_end_index2:
							previous_end_index1 = org_index1
							previous_end_index2 = org_index2
...
##### <TBD> (case, default for switch)
								...
								elif proc_codes.proc_data_list[index1_r].left[index2_r].strip().find('=') == len(proc_codes.proc_data_list[index1_r].left[index2_r])-1:
									#subproc_reverse = False
									if debug_out:
...
# save crrl stat
				...
				if condition_end_index1>=0 and condition_end_index2>=0:
					previous_end_index1 = condition_end_index1
					previous_end_index2 = condition_end_index2
...
#skip 'noctrl' in ctrl_stat_list
				...
				if debug_out:
					print 'Type[%d][%d]<%s>%s CtrlStat[%d]<%s>' % ( \
						index, index2, proc_codes.proc_data_list[index].type[index2], proc_codes.proc_data_list[index].left[index2],\
